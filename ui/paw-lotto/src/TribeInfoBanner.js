...
export default function TribeInfoBanner ({ tribeInfo, yesterdayTribeInfo, weekAgoTribeInfo }) {
    ...
    const dayWeightDifference = Number(tribeSize) - Number(rawToPaw(yesterdayTribeInfo.tribe.total_tribe_size))
    ...
    const totalTribeMembersYesterday =  totalTribeMembers - yesterdayTribeInfo.tribe.total_num_tribe_members
    const totalTribeSign = (totalTribeMembersYesterday > 0 ) ? "+" : ''
    const tribeMembersOverMinYesterday = tribeMembersOverMin - yesterdayTribeInfo.tribe.num_tribe_members
